...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getUnifiedFeed({
    ...
    try {
      ...
      return {'status': 'error', 'items': [], 'next_cursor': null, 'has_more': false};
    } catch (e) {
      return {'status': 'error', 'error': e.toString(), 'items': [], 'next_cursor': null, 'has_more': false};
    }
  }
  ...
  static Map<String, dynamic> _formatFeedItem(Map<String, dynamic> item) {
    ...
    if (type == 'post' || type == 'social_post') return _formatSocialPostFromFeed(item);
    if (type == 'video' || type == 'video_post') return _formatVideoPostFromFeed(item);
    return {
      'type': 'article', 'id': item['id'] ?? 0, 'title': 'Unknown Item',
      'content': 'Content not available.', 'created_at': DateTime.now().toIso8601String(),
      'category': 'Misc', 'category_id': null, 'image_url': null,
      'source_url': '', 'is_ai_rewritten': false,
    };
  }
  ...
  static Map<String, dynamic> _formatArticleFromFeed(Map<String, dynamic> item) {
    ...
    return {
      ...
      'content': content, 'sentiment': item['sentiment'] ?? 'POSITIVE',
      'source_url': item['source_url'] ?? '', 'created_at': item['created_at'],
  ...
  static Map<String, dynamic> _formatSocialPostFromFeed(Map<String, dynamic> item) {
    ...
    return {
      'type': 'social_post', 'id': item['id'].toString(),
      'user_id': item['user_id'] ?? item['author_id'] ?? item['created_by'],
      'author': authorName, 'avatar': authorName.isNotEmpty ? authorName[0].toUpperCase() : 'U',
      ...
      'category_id': -1, 'category': 'Social Posts', 'image_url': item['image_url'],
    };
  }
  ...
  static Map<String, dynamic> _formatVideoPostFromFeed(Map<String, dynamic> item) {
    final authorName = item['author'] ?? item['display_name'] ?? 'Unknown';
    return {
      'type': 'video_post', 'id': item['id'].toString(),
      'author': authorName, 'avatar': authorName.isNotEmpty ? authorName[0].toUpperCase() : 'V',
      'title': item['title'] ?? '', 'content': item['content'] ?? '',
      'created_at': item['created_at'],
      'likes': item['likes_count'] ?? item['likes'] ?? 0,
      'isLiked': item['user_has_liked'] == 1 || item['user_has_liked'] == true,
      'category_id': -2, 'category': 'Video', 'image_url': item['image_url'],
  ...
  static Future<Map<String, dynamic>> getCategories() async {
    try {
      ...
      if (response.data is List) return {'status': 'success', 'categories': response.data, 'count': (response.data as List).length};
  ...
  static Future<Map<String, dynamic>> saveUserPreferencesAuth(List<int> categoryIds, String token) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/user/preferences',
          data: {'category_ids': categoryIds}, options: Options(headers: {'Authorization': 'Bearer $token'})));
      if (response.data is Map<String, dynamic>) return response.data as Map<String, dynamic>;
  ...
  static Future<Map<String, dynamic>> saveUserPreferences(List<int> categoryIds) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/user/preferences', data: {'categories': categoryIds}));
  ...
  static Future<Map<String, dynamic>> searchFriends(String query) async {
    try {
      ...
      if (response.statusCode == 200 && response.data is List) return {'status': 'success', 'data': response.data};
  ...
  static Future<Map<String, dynamic>> postContactsSuggest(List<String> hashedContacts) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/contacts/suggest', data: {'hashed_contacts': hashedContacts}));
  ...
  static Future<Map<String, dynamic>> sendFriendRequest(int userId) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) return {'status': 'success', 'message': 'Friend request sent successfully'};
  ...
  static Future<Map<String, dynamic>> cancelFriendRequest(int userId) async {
    try {
      ...
      final response = await _dio.delete('/friends/$userId/request',
          options: Options(headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json', 'Accept': 'application/json'}));
      if (response.statusCode == 200 || response.statusCode == 204) return {'status': 'success', 'message': 'Friend request cancelled'};
      if (response.statusCode == 404 || response.statusCode == 405) {
        final fallback = await _dio.post('/friends/$userId/cancel',
            options: Options(headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'}));
        if (fallback.statusCode == 200 || fallback.statusCode == 204) return {'status': 'success', 'message': 'Friend request cancelled'};
      }
  ...
  static Future<Map<String, dynamic>> addFriend(String userId) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/friends/add', data: {'user_id': userId}));
      return response.data ?? {'status': 'success'};
    } catch (e) {
      return {'status': 'error', 'error': e.toString()};
    }
  ...
  static Future<Map<String, dynamic>> login(String email, String password) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/login', data: {'email': email.trim(), 'password': password.trim()}));
      final data = response.data;
      if (data != null && data['token'] != null && data['user_id'] != null) {
        await PreferencesService.saveUserData(token: data['token'], userId: data['user_id'], name: data['display_name'] ?? data['name'] ?? '', email: email);
  ...
  static Future<Map<String, dynamic>> register(String displayName, String email, String password, String phoneNumber) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/register', data: {
        'email': email.trim(), 'password': password.trim(),
        'display_name': displayName.trim(), 'phone_number': phoneNumber.trim(),
      }));
      return response.data ?? {'status': 'error'};
    } catch (e) {
      return {'status': 'error', 'error': e.toString()};
  ...
  static Future<Map<String, dynamic>> authenticatedRequest(String endpoint, {
    required String method, required String token,
    Map<String, dynamic>? data, Map<String, dynamic>? queryParameters,
  }) async {
    try {
      final options = Options(method: method.toUpperCase(), headers: {
        'Authorization': 'Bearer $token', 'Content-Type': 'application/json', 'Accept': 'application/json',
      });
      Response response;
      switch (method.toUpperCase()) {
        case 'GET': response = await _dio.get(endpoint, queryParameters: queryParameters, options: options); break;
        case 'POST': response = await _dio.post(endpoint, data: data, queryParameters: queryParameters, options: options); break;
        case 'PUT': response = await _dio.put(endpoint, data: data, queryParameters: queryParameters, options: options); break;
        case 'DELETE': response = await _dio.delete(endpoint, queryParameters: queryParameters, options: options); break;
      ...
      if (response.statusCode == 401) return {'status': 'error', 'error': 'Authentication failed', 'statusCode': 401};
      return {'status': 'error', 'error': 'Server error: ${response.statusCode}', 'statusCode': response.statusCode};
    } catch (e) {
      if (e is DioException && e.type == DioExceptionType.connectionTimeout) return {'status': 'error', 'error': 'Connection timeout'};
      return {'status': 'error', 'error': e.toString()};
    }
  }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // VIDEO FEED
  // ✅ FIX: gotFullPage काढला → infinite loop fix
  // gotFullPage = 20>=20 = ALWAYS TRUE → hasMore ALWAYS TRUE → same videos loop
  // ─────────────────────────────────────────────────────────────────────────
  static Future<Map<String, dynamic>> getVideoFeed({
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data is Map<String, dynamic>) {
        ...
        debugPrint('🎬 VIDEO API: offset=$offset → ${rawVideos.length} videos | has_more=${data['has_more']} total=$total');

        // ✅ FIX: ?? operator false साठी काम करत नाही → explicit == true
        // ✅ FIX: gotFullPage काढला — 20>=20=true → INFINITE LOOP होतं!
        final serverSaysHasMore = data['has_more'] == true || data['hasMore'] == true;
        final totalSaysMore = total > 0 && (offset + rawVideos.length) < total;
        final hasMore = serverSaysHasMore || totalSaysMore;

        debugPrint('📊 hasMore=$hasMore (server=$serverSaysHasMore totalMore=$totalSaysMore)');

        return {'status': 'success', 'videos': rawVideos, 'has_more': hasMore, 'total': total};
      }
  ...
  static Future<Map<String, dynamic>> deleteAccount() async {
    try {
      final token = await PreferencesService.getToken();
      if (token == null) return {'status': 'error', 'error': 'Not authenticated'};
      final response = await _dio.delete('/user/account',
          options: Options(headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json', 'Accept': 'application/json'}));
      if (response.statusCode == 200 || response.statusCode == 204) return {'status': 'success', 'message': 'Account deleted successfully'};
      if (response.statusCode == 401) return {'status': 'error', 'error': 'Authentication failed'};
      return {'status': 'error', 'error': 'Failed to delete account. Please try again.'};
    } catch (e) {
      if (e is DioException && e.type == DioExceptionType.connectionTimeout) return {'status': 'error', 'error': 'Connection timeout. Please check your internet.'};
      return {'status': 'error', 'error': e.toString()};
    }
  }
  ...
  static Future<Map<String, dynamic>> googleMobileLogin(String idToken) async {
    try {
      final response = await _retryRequest(() async => await _dio.post('/auth/google/mobile', data: {'id_token': idToken}));
      final data = response.data;
      if (data != null && data['token'] != null) {
        await PreferencesService.saveUserData(
          token: data['token'], userId: data['user_id'] ?? 0,
          name: data['user']?['display_name'] ?? 'User', email: data['user']?['email'] ?? '',
        );
        await PreferencesService.setOnboardingCompleted(true);
      }
      return data ?? {'status': 'error'};
